...
minikube image load "mvc-demo:${TAG}"

echo "[dev-env] creating mvc-demo namespace"
kubectl create namespace mvc-demo

ATTEMPTS=0
ROLLOUT_STATUS_CMD="kubectl get namespace mvc-demo -n mvc-demo"
...
kubectl create -f manifests/persistent-storage-no-pvc-deletion.yaml -n mvc-demo

ATTEMPTS=0
ROLLOUT_STATUS_CMD="kubectl rollout status deployment.apps/rfs-redisfailover-persistent-keep -n mvc-demo"
until $ROLLOUT_STATUS_CMD || [ $ATTEMPTS -eq 60 ]; do
  ATTEMPTS=$((ATTEMPTS + 1))
  sleep 10
done
...
STATEFULSET_REPLICAS=$(kubectl get statefulset rfr-redisfailover-persistent-keep -o jsonpath='{.spec.replicas}' -n mvc-demo)
ATTEMPTS=0
until [[ ${STATEFULSET_REPLICAS} -eq $(kubectl get statefulset rfr-redisfailover-persistent-keep -o jsonpath='{.status.readyReplicas}' -n mvc-demo) ]] || [ $ATTEMPTS -eq 60 ]; do
  ATTEMPTS=$((ATTEMPTS + 1))
  sleep 10
done

echo "[dev-env] installing mvc-demo charts"
helm upgrade --install \
    mvc-demo charts/mvc-demo \
    --namespace mvc-demo \
    --set ingress.host.name="mvc-demo.${INGRESS_HOST}"

ATTEMPTS=0
ROLLOUT_STATUS_CMD="kubectl rollout status deployment/mvc-demo -n mvc-demo"
until $ROLLOUT_STATUS_CMD || [ $ATTEMPTS -eq 60 ]; do
  ATTEMPTS=$((ATTEMPTS + 1))
  sleep 10
done
...
FastAPI available under: http://mvc-demo.${INGRESS_HOST}/
